...
for (int i = 0; i < MaxSteps; i++)
{
    ...
    if( cursample > 0.001)
    {
		float cursampleNoise = PseudoVolumeTexture(NoiseTex, TexSampler, CurPos*NoiseScale, XYFrames, numFrames).r;
		
		cursample+=cursampleNoise*NoiseMul;
		if( cursample > 0.001)
		{
			if(Distance==0)
			{
			...
			shadowterm*=(1-exp(-shadowdist*ShadowDensity*2));	
